...
출시 전 검토 자료: PR #27의 시바견·오리·똥·떡볶이·쿼카 5종은 [character-five 검토 패키지](reviews/character-five/README.md)에서 원본과 후보를 구분한다. 서면 계약은 사용자 확인으로 완료다. 시바·오리·똥·떡볶이는 원작 외형을 유지하면서 발 연결·동작 중복을 보정하고, 쿼카는 동물 정체성을 유지한 새 외형을 검토한다. 선택 물건은 테니스공·기존 삑삑 오리·휴지 뭉치·어묵꼬치·잎사귀이며 캐릭터와 별도 판매하는 정책을 유지한다. 제시 콘셉트 승인과 실제 24×24 캐릭터·16×16 물건 회전/충돌·음원 승인은 구분한다. 사용자 요청에 따라 최종 승인 전에도 새 쿼카를 포함한 90프레임과 물건별 충돌음·합성 후보를 먼저 제작하며, 로컬 검토 페이지의 5종 동작 카드와 물건별 직접 소리 재생 버튼으로 검토한다. 발신자·수신자 설정과 충돌 횟수 표시는 제거한다. 쿼카는 작은 눈·파란 목도리로 재작업하며, 기존 삑삑 오리의 그림·승인 꽥 음원을 오리의 애착 물건으로 그대로 재사용한다. 테니스공·어묵꼬치 소리는 각 3개 후보로 다시 검토한다. 캐릭터·물건별 고유 설명은 짧은 유머를 담은 총 10개 문구를 파일과 해시로 승인받아 후속 배포 명세에 포함한다. 이번 요청의 결과물은 자산 저장·문서화·승인 PR이며 앱 연결 코드 구현은 포함하지 않는다. 전체 승인 후에도 이번 결과물은 출시 대기 자료이며 현재 지원 콘텐츠 수에 포함하지 않는다. 이후 macOS 업데이트 지시 전에는 활성 manifest·앱·상점·서버·플랫폼 배포 사본·버전·릴리스에 연결하지 않는다.
