...
현재 공개 버전은 [macOS manifest](../release/macos.json)와 [Windows manifest](../release/windows.json)를 따른다. App Store 후보는 Xcode 타깃 설정에서 확인한다. 현재 판매 상품·가격·현재 및 복원용 Apple ID는 [상품 원본](../assets/v1/commerce-catalog.json)을 따른다. 단계별 승인·재등록·업로드 이력은 [결정 기록](DECISIONS.md)과 [종료된 운영 기록](history/product-operations.md)에 보존한다.
...
- macOS 설정과 메뉴바에서 공통 catalog의 유료 24종(캐릭터 7·말풍선 3·투척물 14)을 제공하되 direct production 구매는 잠근다. 격리된 Sidey-dev만 Google identity를 연결한 뒤 PortOne 테스트 결제를 시작한다.
- direct판 카탈로그는 승인된 말풍선 3종과 투척물 14종을 포함하되 production의 `.comingSoon`과 서버 `sales_enabled=false`는 유지한다. 결제·지급·자동 장착·해제·환불 회수는 Sidey-dev와 staging에서만 검증한다.
- direct판과 App Store판의 `내 프로필`은 말풍선·투척물 선택 영역을 항상 표시하고, 보유 상품이 없어도 `기본 말풍선`과 `기본 말랑공`을 각각 장착 상태로 보여준다. snapshot의 활성 entitlement로 확인한 상품만 종류별로 추가한다. 선택은 프로필 저장과 분리해 즉시 계정 전체 그룹에 적용하며 direct production의 상점 조회·구매 잠금과 관계없이 서버 장착 RPC를 사용할 수 있다.
...
- App Store판은 공통 catalog의 24종을 StoreKit 2 비소모성 상품으로 제공하며 서버가 Apple JWS와 App Store Server API 결과를 검증한 뒤에만 entitlement를 지급한다. 한국 가격은 기존 catalog 가격과 일치하는 App Store 가격 포인트를 우선하고 없으면 바로 위 가격 포인트를 사용하며, 실제 표시는 StoreKit 현지화 가격을 따른다.
...
- 두쫀쿠 충돌은 승인 시트 8–11번 프레임을 각각 0.08·0.10·0.28·0.12초 재생해 늘어나는 속이 30 FPS에서도 구분되게 한다. 다른 투척물의 0.24초 충돌과 사운드는 유지한다. 미리보기 종료·방 변경·잠금 시 충돌 노드와 동작을 함께 정리한다.
- 현재 판매 후보 24종은 `assets/v1/commerce-catalog.json`을 기준으로 한다. 캐릭터 7종·말풍선 3종·투척물 14종이며, 투척물 중 애착 물건 7종은 각각 직배포 990원 / 한국 App Store 1,100원이다. 기존 캐릭터 포함 상품 4종의 App Store ID를 복원용으로 유지하므로 검증 ID는 총 28개다. 신규 단품 구매에는 애착 물건이 포함되지 않는다. 이전 포함 상품과 전환 전 기존 지급의 물건 권리는 출처별로 보존한다.
...
- Windows 연결 프로필의 변경을 OS 이벤트로 감시한다. 경로 없음이 확인되면 회색 연결 대기를 표시하고 소켓 재접속·진행 중 복구 조회를 취소한다. `LocalAccess`·제한된 인터넷 연결은 서버 접속을 허용하며 OS의 인터넷 확인 결과만으로 연결 완료나 영구 차단을 판정하지 않는다. 서버 장애에는 최대 5분 대기의 자동 복구를 유지한다. 연결 상태 버튼으로 수동 재시도하며, 인증·권한 문제로 중단했다면 세션과 그룹 목록을 재확인한다. 초기화 실패도 같은 버튼에서 재시도할 수 있고 이미 진행 중인 초기화는 합친다.

- 실시간 연결 시작 시 독립 채널 구독을 병렬로 요청하고 성공 응답 뒤에만 연결 완료로 전환한다. 소켓 단절 시 응답 대기를 즉시 해제하며, 첫 재접속은 350ms 후 시작하고 연속 실패는 아래 Windows backoff 정책을 따른다. 초기 구독 실패에도 활동 감시 등 나머지 초기화를 계속한다. 그룹 조회와 구독은 별도 단계이므로 조회 완료 시각을 연결 완료 시각으로 간주하지 않는다.

- 온보딩 완료·오버레이 표시 설정·저장된 닉네임이 있으면 시작 직후 저장된 내 캐릭터를 회색 상태 점의 연결 대기 상태로 표시한다. 인증 복원·그룹 조회·메시지 이력 수신을 기다리지 않는다. 서버 스냅샷 수신 시 실제 그룹 멤버로 교체하고 Realtime 연결 전에는 회색 상태 점으로 표시한다. 다른 멤버의 로컬 캐시는 만들지 않으며, 서버에서 받은 멤버 목록과 오프라인 멤버 표시 설정을 사용한다. 초기 연결 실패에도 로컬 표시를 유지하며, 저장된 숨김 설정과 온보딩 흐름은 존중한다. 로컬 시작 화면은 프로필·그룹·권한을 생성하지 않고 투척·메시지 이벤트를 보내지 않는다.
- 닉네임 상자의 위치는 캐릭터 이미지의 외곽이나 기절 여부 대신 발 기준 고정 간격으로 결정한다. 오버레이 네 가장자리와 상점에 같은 기준을 적용하고, 닉네임 길이에 따른 상자 폭은 유지한다. 기절 별·링의 공간을 확보하며 닉네임보다 앞에 그려 다른 캐릭터의 닉네임에도 가려지지 않게 한다.
- 실제 충돌을 대상 UUID별 10초 동안 집계해 10회에서 6초 기절한다. 기절 중 추가 충돌은 연장·재누적하지 않는다. 이동·확대·새 투척을 막고 채팅·이미 발사한 물체는 유지한다. 방 전환·멤버 제거·연결 단절·오버레이 종료 시 상태를 정리한다. 수면 자세·노란 별 3개·링을 기존 픽셀 렌더러로 표시하고 Zzz·평상시 반짝임을 숨긴다.
- 승인 WAV 15개를 Windows `Assets/Impacts/<투척물 ID>/<투척물 ID>.wav`에 원본 그대로 보관하고 설치 루트의 같은 상대 경로 한 곳에 배포한다. 오버레이와 상점의 실제 충돌에서 재생하며 최대 4개 동시 재생·80ms 시작 간격·500ms 초과 요청 폐기를 적용한다. 설정 OFF·잠금·절전·출력 장치 변경·종료 시 소리를 정리한다. 오류는 해당 재생만 생략한다.
- 설정의 소리 카드는 음량 레이블과 스피커 아이콘 음소거 버튼·현재 음량·0–100% 슬라이더를 한 줄에 배치한다. 별도 토글 스위치와 설명 문단은 없애며, 아이콘 버튼은 음소거 상태에 따라 모양과 접근성 이름을 바꾼다. 기본 상태는 소리 켜짐이다. 음원 선택·별도 미리듣기 버튼은 두지 않고 상점에서 실제 투척 충돌로 소리를 확인한다. 사용자가 음량을 변경한 뒤 슬라이더에서 포인터를 놓을 때만 기본 말랑공 피격음을 최종 음량으로 한 번 재생한다. 드래그 중·조절 도중 잠시 멈춘 상태·설정 저장 완료에서는 재생하지 않는다. 키보드 조절은 키를 놓을 때 재생하며, 중복 완료 이벤트는 다시 재생하지 않는다. 초기 로드·상태 동기화·저장 실패 복구에서는 소리를 내지 않고 0%·설정 화면 이탈·창 닫기에는 피드백을 정리한다. 슬라이더 0% 변경은 음소거, 양수 변경은 소리 켜짐으로 연결하며 음소거 중에도 슬라이더 조절은 가능하다. 버튼으로 직접 음소거할 때는 음량을 유지하고, 0%에서 켜면 이번 실행의 마지막 양수 음량 또는 기본 100%로 복원한다. 조용히 모드와 독립적으로 동작한다. 음량은 SIDEY 효과음에만 적용하고 Windows 전체 음량은 변경하지 않는다. Windows preferences schema 4의 `characterSoundEffectsEnabled`는 누락 시 true, `characterSoundEffectsVolume`은 누락 시 100이다. 저장된 false와 0%를 유지하고 범위를 벗어난 음량은 0–100으로 제한하며, 로드 시 0%이면 토글도 OFF로 정규화한다. 음소거 여부와 음량은 함께 저장한다. 드래그 중 음량은 즉시 반영하고 설정 저장은 잠시 묶어서 처리하며, 저장 실패는 이전 상태로 복구한다.
- 프로필 장착은 종류별 요청 대상과 확정 선택을 분리한다. 요청 타일에 ProgressRing을 표시하고 성공 전까지 기존 체크를 유지한다. 동일 종류만 요청 중 비활성화하며 오류·지연 응답에서도 초안·포커스·확정 선택을 보존한다.
- Windows 애니메이션 설정을 실행 중 감시한다. OFF이면 상점은 정지 장면과 완성된 말풍선 예시를 표시하고 반복 타이머·수동 확대/투척·미리보기 소리를 정리한다. ON 복귀는 첫 장면부터 시작한다. 기절은 고정 수면 프레임·별·링, 장착 진행은 정적 표식·텍스트로 표시한다. 일반 오버레이 산책의 기존 정책은 유지한다. 오버레이를 새로 만들 때 ON이면 `UpdateLayeredWindow`의 전체 alpha를 약 0.24초 동안 ease-out으로 올리고 OFF이면 첫 frame부터 완전 불투명하게 표시한다. 작업 표시줄 표시·숨김에 따른 edge inset은 ON일 때 기존 속도로 부드럽게 보간하고 OFF이면 다음 30 FPS frame에 목표 위치로 즉시 맞춘다.
...
- Windows 1.1.0 개발 빌드는 staging Supabase가 확인된 경우에만 기존 유료 캐릭터 4종의 Google identity 연결과 PortOne test checkout을 활성화한다. production project ref, Release 빌드 또는 명시적 개발 commerce opt-in이 없는 실행에서는 연결·주문 action을 만들지 않는다.
...
- Windows 1.1.0 Release는 `sidey://auth/google` callback 전달 기반과 설치기 등록만 준비하고 상점은 10개 상품의 비활성 `구매 준비 중` 상태를 유지한다. 서버 응답이나 환경변수만으로 판매를 열 수 없고, 향후 새 클라이언트 버전의 명시적 빌드 변경과 서버 판매 활성화를 함께 거쳐야 한다.
...
- Windows 기본 CTA는 현재 정식 v1.2.1의 고정 Setup EXE를 직접 가리키며 저장소에서부터 활성 링크로 제공한다. Pages Actions는 버전에 맞는 정식 Release의 단일 Windows 설치 자산과 고정 URL을 확인한 경우에만 배포를 계속한다.
...
- 랜딩은 보라·분홍 방사형 그라데이션과 잔잔한 점 패턴의 기존 밝은 배경을 사용한다. 랜딩·상점·정책·결제·컨트리뷰터 도구는 공통 Astro 상단 바를 사용한다. 상단 바는 화면 위 16px에 고정한 내용 맞춤 폭·높이 64px의 중앙 캡슐형이며, 내용 둘레에 여백을 두고 반투명 흰색 배경과 7px 배경 블러를 적용한다. `SIDEY` 브랜드 자간은 좁게 두고 메뉴 글자는 본문보다 또렷하게 보이도록 키우며, 메뉴를 `기능·다운로드·자주 묻는 질문`, 세로 구분선, `새로운 기능·상점·GitHub↗` 순서로 통일한다. 상점은 드롭다운 없이 `/store/`로 연결하고 영어 화면에서도 한국어 상점으로 연결한다. 첫 화면 문구는 `화면에 친구가 총총.`으로 시작하며, 플랫폼별 버전 보조 문구는 첫 화면과 하단 브랜드에서 반복하지 않고 다운로드 섹션에만 둔다. 첫 화면 CTA는 사용자 운영체제를 감지하는 기존 분할형 다운로드 버튼 하나를 유지한다. macOS 선택은 원래 버튼 배경·Apple 아이콘을 유지한 App Store 다운로드 문구로 표시하고 `https://apps.apple.com/kr/app/sidey/id6808528060?mt=12`로 연결한다. Windows에서는 Windows 다운로드를 표시하며 옆 메뉴에서도 App Store·Windows를 직접 선택할 수 있다. 하단 다운로드 섹션의 직배포 파일·해시 안내는 유지한다. 제공받은 플랫폼 로고를 쓰되 Windows 표식은 Apple 표식과 시각적 크기가 같도록 작게 보정하고 별도의 다운로드 동작 아이콘은 두지 않는다. 모든 다운로드 버튼을 가리키면 설치로 라이선스 계약에 동의한다는 문구를 보여 준다. 다운로드 섹션의 CTA는 `macOS 다운로드`와 `Windows 다운로드`로 표기한다. 플랫폼 표기는 `macOS`와 `Windows`의 고유 대소문자를 유지하고 SHA-256은 각 지원 환경 문구 바로 아래에 작은 보조 정보로 표시한다. 첫 장면은 얇은 베젤과 같은 짙은 회색을 쓰는 폭이 좁은 직사각형 모니터 목·가로 받침, `오늘 할 일 | 메모` 브라우저를 사용하고 캐릭터·이름표·말풍선은 64px 캐릭터 비율에 맞춰 줄이며 보이는 발끝을 브라우저 하단선에 맞춘다. 콩이는 보리 바로 왼쪽의 15% 지점에서 오른쪽으로, 토리는 오른쪽 벽에서 왼쪽으로 출발한다. 토리의 입력 중 말풍선은 콩이의 말풍선 표면·꼬리·글자 크기를 공유하는 42×30 영역에서 0.35초 간격으로 `.`, `..`, `...`을 가운데 정렬해 반복한다. 두 캐릭터는 앱의 기본 가속 32pt/s²·감쇠 0.82·최대 22pt/s를 따르고, 캐릭터와 겹치면 추가 전진 가속 64pt/s²·감쇠 0.92·최대 30pt/s의 슬라이딩으로 통과한 뒤 기본 이동으로 복귀하며 좌우 벽에서 방향을 반전한다. 첫 장면 다음에는 검증된 실제 macOS 작업 화면과 세 단계 설명을 한 화면에서 함께 읽는 컴팩트한 2열 사용 장면을 먼저 보여 주며, 좁은 화면에서는 이미지와 설명을 한 열로 배치한다. 그 다음 기능 섹션은 표 대신 평소 상태, 말랑공 상호작용, 크게 인사의 세 카드로 구성하고 서로 다른 제목 높이와 관계없이 세 장면의 바닥선을 카드 하단에 맞춘다. 말랑공은 0.2초 뒤 빙수에게서 출발해 거리 기반 높이의 이차 베지어 포물선으로 토리에게 닿고, 도착 순간 토리의 4프레임 hit과 말랑공의 4프레임 impact를 0.44초·0.24초 동안 각각 재생한다. 크게 인사는 24px 원본 아래쪽에서 3px 위인 발 기준점을 0.2초 확대·0.6초 복귀 동안 고정하고 캐릭터만 7배로 확대한다. 다운로드 다음에는 앱·지원 환경·비공개 그룹·개인정보·메시지 보관·오버레이 제한을 설명하는 접이식 자주 묻는 질문을 둔다. 추가 프로그램 캡처가 생기기 전에는 가짜 UI 화면을 만들지 않는다. 캐릭터·동작·투척물은 GitHub Pages의 `/SIDEY/assets/` 경로에서 불러온다.
- Astro의 `i18n` 설정은 `ko`, `en`, `ja` 모두 URL 접두사를 사용하도록 선언한다. 로케일별 파일 기반 route는 `Astro.currentLocale`을 읽고 canonical·alternate·내비게이션 URL은 `astro:i18n` helper로 만든다. 정적 Pages에서 서버 측 선호 로케일을 읽을 수 없으므로 콘텐츠가 없는 로케일 중립 진입 페이지에서 브라우저의 첫 번째 선호 언어를 읽어 같은 하위 경로의 `/ko/`, `/ja/` 또는 `/en/` 페이지로 이동한다. 랜딩은 layout·hero·showcase·motion·download·FAQ·footer와 공통 상단 바·다운로드 버튼 컴포넌트로 나누고 세 언어의 사용자 문구는 공용 TypeScript UI 사전에서 관리한다. 영어와 일본어 문구는 각 언어권에 자연스러운 제품 문구로 별도 작성한다. 한국어·영어는 `Asta Sans`, 일본어는 `M PLUS 1p`를 사용하되 브랜드 표기는 `Asta Sans`로 유지한다. 정적 표현 스타일은 `src/styles` 아래 SCSS에만 두고 공통 설정과 상점·새로운 기능·FAQ·정책·결제 규칙을 Sass partial로 분리한다. 배경 위 주요 카드의 반경은 1.5rem이며, 다운로드 CTA는 반투명 흰색 블러 표면·얇은 테두리·원형 아이콘을 갖는 캡슐형이다. `#features`와 `#download`, `#faq` 내비게이션은 현재 읽고 있는 섹션에 `aria-current="location"`을 적용한다.
- 700px 이하에서 공통 상단 바의 햄버거 메뉴를 열면 동일한 18px 블러 표면이 닫힌 상단 바와 같은 위 16px·좌우 8px 위치를 유지한 채 아래 16px까지 확장되고 문서 스크롤을 잠근다. 메뉴의 `#features`·`#download`·`#faq`는 패널을 닫은 뒤 같은 문서의 대상을 직접 스크롤한다. 공개 페이지는 저장된 사용자 선택이 없을 때 시스템 `prefers-color-scheme`에 따라 밝은 색과 어두운 색을 자동 선택한다. 공통 상단 바에는 GitHub 링크 뒤 구분선과 원형 테마 전환 버튼을 두고, 선택한 테마는 브라우저에 저장한다. 모바일에서는 테마 버튼 앞 구분선을 숨긴다. 버튼 hover는 아이콘 색을 바꾸지 않고 원형 배경으로 나타내며, 모션 감소 설정이 아닐 때 테마 화면을 짧게 교차 전환한다. 모든 다운로드 CTA의 안내에는 이용약관과 개인정보 처리방침 링크를 주변 문구와 같은 색·장식으로 표시한다. 한국어 `/ko/terms/`·`/ko/privacy/`·`/ko/refund/`를 원문으로 유지하고 `/en/terms/`·`/en/privacy/`·`/en/refund/`에는 번역본 안내와 한국어 원문 링크가 있는 영문 문서를 제공한다.
...
- `main`의 웹 파일 또는 Pages 워크플로가 바뀌면 GitHub Actions가 Astro를 고정된 lockfile로 설치해 `website/dist`를 빌드·검증하고 현재 macOS·Windows 정식 Release를 확인한다. 정식 DMG와 Setup EXE를 다시 내려받아 계산한 SHA-256을 `/ko/`, `/en/`, `/ja/` 다운로드 화면에 반영하고, 같은 Windows 해시로 업데이트 manifest를 `dist` 사본에 추가한 뒤 그 결과만 Pages artifact로 올려 `github-pages` 환경에 배포한다. macOS 배포 스크립트와 Windows 정식 릴리스 성공 시에도 같은 Pages 워크플로를 직접 호출한다. custom domain과 별도 Sites 호스팅은 사용하지 않는다.
...
- Windows는 저장된 Supabase 익명 세션을 먼저 복구하고, 세션이 없는 신규 설치에서만 새 익명 계정을 만든다. access·refresh token과 평문 초대 코드는 Windows Credential Manager에 보관한다. 1.1.0 개발 빌드는 staging의 PortOne 테스트 결제를 시작할 때만 PKCE S256과 `sidey-dev://auth/google` callback으로 같은 사용자 UUID에 Google identity를 연결한다. Release는 `sidey://auth/google` callback 기반만 설치하고 연결·구매 UI와 주문 호출을 컴파일 타임으로 잠근다.
...
캐릭터 물체 던지기는 `더블 우클릭 후 던지기` 설정으로 입력 방식만 바꾼다. 기본값은 OFF이며 화면에 표시되는 다른 멤버의 52×52 hotspot을 Presence 상태와 무관하게 항상 표시해 한 번 클릭하면 즉시 던진다. ON이면 평소 친구 hotspot을 만들지 않고 내 캐릭터를 더블 우클릭한 뒤 10초 동안만 표시한다. 더블 우클릭을 다시 하면 10초로 갱신되고 활성 시간에는 여러 친구에게 계속 던질 수 있다. 두 방식 모두 송신자 기준 0.5초 쿨타임을 적용한다. 온라인·자리 비움·오프라인·재연결 캐릭터를 모두 대상으로 허용하며 자기 자신만 제외한다.
...
- 목적지와의 거리가 3pt 미만이면 0.8~3초 쉬고 새 로컬 목적지를 고른다. 초기 휴식은 0~1.5초다. 정지 상태에서는 기존 목적지를 보존하고, 화면 배율과 무대 확대율을 반영해 이동 속도·도착 거리를 같은 논리 단위로 계산한다. 걷기 표시는 속도가 2pt/s를 넘고 휴식·정지 중이 아닐 때만 사용하며, 방향 반전은 새 목적지 선택이 아니라 실제 이동 방향을 따른다.
- Windows 상점 캐릭터 미리보기도 같은 산책 규칙을 사용하며 벽에서 즉시 속도를 반전하는 별도 규칙을 두지 않는다. 상품 캐릭터 더블클릭은 발 기준 3배 확대·복귀, 친구 캐릭터 클릭은 시그니처 투척을 재생한다. 투척은 출발 시점의 캐릭터 중심·비행시간·호 높이를 고정하고 움직이는 친구의 중심을 따라간다. 피격된 친구는 0.44초 멈추고 기존 목적지로 복귀한다. 별빛 우파루파의 평상시 반짝임과 확대 시 중앙 섬광·두 겹 반짝임을 표시하고, 미리보기를 닫으면 타이머와 상호작용을 중지한다. 모든 미리보기 이벤트는 서버에 전송하지 않는다.
...
수달·돼지·나무와 고유 투척물(조개·돼지고기·작은 나무), 별도 투척물(눈송이·야구공·왁뿌볼·두쫀쿠)은 [3차 검토](../assets/candidates/otter-pig-tree/round-3/README.md)의 시각 승인을 받았다. 왁뿌볼 음원은 A로 고정하며 돼지고기 `촵` 2차(0.24초)까지 사용자 승인을 받았다. macOS 직배포판·App Store판 양쪽 캐릭터 목록·고유 투척물·장착 가능 유료 투척물·충돌음에 연결한다. 수달·돼지·눈송이·야구공은 각각 직배포 990원 / App Store 1,100원, 나무·왁뿌볼·두쫀쿠는 각각 1,900원 / 2,200원이다. 애착 물건은 별도 판매하며 기존 포함 구매 권리는 보존한다. 나무는 우클릭으로 정지·걷기를 전환하고 더블 우클릭으로 던지기를 활성화한다. 정지 중 호흡·피격·던지기 동작은 유지하며 수평 반전은 하지 않는다. Sidey-dev Debug 로컬 테스트 방에서도 실제 앱 렌더러·충돌음·나무 정지를 검토할 수 있다. App Store판에 효과음 설정과 재생·중지 정책을 동일하게 적용하며 기절 기능은 직배포판에만 유지한다. Windows 적용과 운영 판매 잠금 해제는 이번 작업에 포함하지 않는다.

출시 전 검토 자료: PR #27의 시바견·오리·똥·떡볶이·쿼카 5종은 [character-five 검토 패키지](reviews/character-five/README.md)에서 원본과 후보를 구분한다. 서면 계약은 사용자 확인으로 완료다. 시바·오리·똥·떡볶이는 원작 외형을 유지하면서 발 연결·동작 중복을 보정하고, 쿼카는 동물 정체성을 유지한 새 외형을 검토한다. 선택 물건은 테니스공·기존 삑삑 오리·휴지 뭉치·어묵꼬치·잎사귀이며 캐릭터와 별도 판매하는 정책을 유지한다. 제시 콘셉트 승인과 실제 24×24 캐릭터·16×16 물건 회전/충돌·음원 승인은 구분한다. 사용자 요청에 따라 최종 승인 전에도 새 쿼카를 포함한 90프레임과 물건별 충돌음·합성 후보를 먼저 제작하며, 로컬 검토 페이지의 5종 동작 카드와 물건별 직접 소리 재생 버튼으로 검토한다. 발신자·수신자 설정과 충돌 횟수 표시는 제거한다. 쿼카는 작은 눈·파란 목도리로 재작업하며, 기존 삑삑 오리의 그림·승인 꽥 음원을 오리의 애착 물건으로 그대로 재사용한다. 테니스공·어묵꼬치 소리는 각각 후보 1로 확정했다. 제시된 캐릭터·물건 외형/동작과 설명 10개는 승인되었으며, 휴지는 새 마찰음 2(audio-v3 B), 잎사귀는 1(audio-v3 A)로 최종 승인했다. [승인 파일 목록](reviews/character-five/FINAL_ASSETS.md)에 선택 시트·충돌음·설명을 정리했으며 출시 대기 패키지로 저장한다. 캐릭터·물건별 고유 설명은 짧은 유머를 담은 총 10개 문구를 파일과 해시로 승인받아 후속 배포 명세에 포함한다. 이번 요청의 결과물은 자산 저장·문서화·승인 PR이며 앱 연결 코드 구현은 포함하지 않는다. 전체 승인 후에도 이번 결과물은 출시 대기 자료이며 현재 지원 콘텐츠 수에 포함하지 않는다. 이후 macOS 업데이트 지시 전에는 활성 manifest·앱·상점·서버·플랫폼 배포 사본·버전·릴리스에 연결하지 않는다.

### 4.4.1 효과음과 직배포판 기절

macOS 두 타깃은 현재 투척물 catalog의 승인 피격음을 실제 충돌 시 한 번 재생한다. 발사·비행·확대·채팅 소리는 재생하지 않는다. 로컬 `캐릭터 효과음` 스위치는 기본 ON이며 끄면 현재 소리를 중단한다. 동시 재생은 최대 4개, 시작 간격은 최소 80ms다. 숨김·잠금·절전·그룹 전환·미리보기 종료 시 예약과 재생을 취소한다.

기절은 직배포판에만 적용한다. 대상별 최근 10초 충돌에서 중복 이벤트를 제외한 10번째 피격에 6초 기절하며 누적을 비운다. 기절 중 추가 충돌은 효과·소리만 재생하고 시간을 연장하지 않는다. 자기 확대·던지기는 차단하되 메시지·타이핑·Presence는 유지한다. 외형 변경은 누적을 유지하고 방 전환·대상 제거·잠금·연결 재수립은 임시 상태를 정리한다. 로컬 충돌 시각을 사용하므로 다른 클라이언트와 기절 시각이 다를 수 있다.

기절 시 offline 몸동작과 승인 별·링을 사용하며 산책·Zzz·ambient sparkle을 멈춘다. 모션 감소에서는 정적 포즈와 별을 표시한다. 회복 시 당시 Presence 모션과 이전 산책 목적지로 복귀한다. 장면이 피격·기절 상태를, 오디오 서비스가 재생·출력 장치·정리를 소유한다. Debug `SIDEYFeedback`은 서버 방을 만들지 않고 실제 렌더러를 검토한다.

### 4.5 말풍선·투척물

모든 신규 상품은 계정 귀속 비소모성 영구 해금이다. 장착은 모든 그룹에 공통이며 구매 승인 직후 해당 종류의 상품을 자동 장착한다. macOS에서 말풍선 장착 해제는 기본 흰 말풍선, 투척물 장착 해제는 공통 말랑공으로 돌아간다. 애착 물건 7종은 독립 상품이며 과거 포함 구매자의 권리는 지급 출처별로 보존한다. Windows의 기존 시그니처 fallback은 해당 플랫폼 구현 계약을 유지한다.
...
Windows 오버레이와 상점 미리보기의 ambient 입자는 macOS처럼 캐릭터 중심 기준 접선 -25~25pt·화면 안쪽 5~37pt에서 시작해 4pt 떠오른다. 화면 가장자리의 회전과 표시 배율을 반영하며, 정지한 온라인 캐릭터도 표시하고 입자별·주기별 위치를 분산한다. 피격 연출은 별도 확대 burst가 아니라 캐릭터 hit 4프레임과 투척물 impact 4프레임을 사용한다. 미리보기의 캐릭터·투척·충돌 프레임은 미리 로드한 표면을 최근접 보간으로 합성하고, 확대 중에도 닉네임을 앞에 표시한다. 종료 시 표면 참조와 그래픽 자원을 해제한다.

Windows UI 문구는 `ko-KR`·`en-US`·`ja-JP`·`zh-CN`·`zh-TW`·`uk-UA`·`ru-RU` 카탈로그를 사용하며 처음에는 시스템 UI 언어를 감지한다. 중국어 싱가포르·Hans는 간체로, 홍콩·마카오·Hant는 번체로 대응한다. 정보 페이지의 서버 연결 상태 바로 다음에는 언어 아이콘과 설명 왼쪽, 한국어·English·日本語·简体中文·繁體中文·Українська·Русский 드롭다운 오른쪽의 단일 행을 두며 별도 언어 섹션 제목과 설명은 추가하지 않는다. 비영어권 언어의 행 제목에는 `/ Language` 보조 표기를 유지한다. 선택한 언어를 로컬 설정에 저장한 뒤 재실행 없이 문구만 갱신하며 창·오버레이·연결과 작성 중인 초안, 현재 선택을 유지한다. 저장 실패 시 기존 선택과 언어를 유지한다. 언어 바로 다음에는 `Windows 기본값`, `밝게`, `어둡게` 순의 테마 선택 행을 두고 선택한 테마를 즉시 적용해 로컬 설정에 저장한다. 저장된 언어 선택이 없으면 `SIDEY_LANGUAGE`로 로컬 검증 언어를 지정할 수 있고 다른 언어는 한국어로 fallback한다. 앱의 미리보기 닉네임과 예시 메시지도 선택한 언어를 사용한다. 앱 시작 전 런처의 오류 안내는 시스템 UI 언어 또는 검증 환경 변수를 따른다. 언어별 키와 포맷 인자 일치를 자동 검사하며, 상점 미리보기에 별도의 더블클릭 조작 안내 문장은 표시하지 않는다. macOS 구현과 Windows 설치 약관의 한국어 원문은 이 변경으로 수정하지 않는다.

Windows 그룹 페이지는 활성·비활성 모두 같은 기본 사람 아이콘을 사용하며 모양과 내부 채움은 유지한다. 그룹 전환 상태에 맞춰 활성은 민트색, 비활성은 회색으로 색상만 바꾼다. 미니 대포 발사대는 네 가장자리에서 상대를 향하도록 회전·반전하고 macOS와 같이 캐릭터 중심에서 진행 방향 6pt·바깥쪽 1pt에 배치한다. 미리보기 발사체·발사대·충돌 효과는 화면 배치 크기를 유지하고 투명도로 표시를 전환해 프레임 사이에 사라지거나 깜빡이지 않도록 한다.

## 5. 메시지와 타이핑

Windows 영어·일본어·중국어 간체/번체·우크라이나어·러시아어는 단어별 직역보다 각 언어 문화권 사용자에게 자연스러운 앱 문구와 동물 이름, 일상 대화를 사용하되 기능·제한·결제 의미를 바꾸지 않는다. 날짜·시간은 선택한 앱 언어가 아닌 컴퓨터의 지역 형식(날짜 순서, 구분자, 12·24시간 표기 등)과 시간대 설정을 따른다. 숫자는 선택한 언어의 표준 형식을 사용한다. 상품 가격은 언어와 무관하게 실제 결제 통화인 원화로 표시하며 환율을 추정해 바꾸지 않는다.

Windows 미리보기의 캐릭터 확대는 최근접 보간 표면을 유지한다. 투척물·발사대·충돌 효과는 크기별로 미리 디코딩한 기본 이미지 컨트롤을 프레임마다 고정하고 투명도만 전환한다. 비행 중 이미지 원본이나 합성 표면을 교체하지 않으며 종료 시 이미지 참조와 디코딩 자원을 해제한다. 내부 스모크에서는 실제 ContentDialog 안의 자체 미리보기 캔버스만 렌더링해 효과 표시 전후의 픽셀 차이를 검사한다. 데스크톱이나 다른 앱 화면을 캡처·저장하지 않는다.
...
- Windows 클라이언트도 상시 폴링 대신 OS 네트워크 이벤트를 사용한다. 경로 단절은 즉시 반영하고 복구 이벤트 뒤 350ms가 지나면 재연결을 시작한다. 기존 5초 heartbeat 전송 주기는 유지하며 수신 무응답 15초 이상을 다음 watchdog 검사에서 연결 이상으로 판정한다. 경로 복구·연결 대기 중 경로 변경·절전 복귀·수동 재시도는 backoff를 건너뛴다. 정상 연결 중 일반 네트워크 상태 알림에는 소켓을 유지하고, 절전 복귀에는 새 소켓과 구독 응답으로 확인한다. 연속 실패의 기준 대기는 첫 350ms 후 8·16·30·60·120·240·300초이며 두 번째부터 ±20% 무작위 지연, 실제 300초 상한을 적용한다. 세션 부재와 세션 요청의 400·401·403 오류는 자동 복구를 중단한다. 채널 인증·권한 거부는 세션 재조회 후 한 차례 재접속하고 반복 거부 시 중단한다. 수동 재시도나 그룹 구독 대상 변경으로 중단을 해제하며, 네트워크 알림·절전 복귀만으로 권한 오류를 반복하지 않는다. 서버 원문 오류나 토큰·프로필·SSID·주소는 진단에 남기지 않는다.
...
월드 패널 전체는 `ignoresMouseEvents`로 클릭을 통과시킨다. 내 캐릭터 위의 52×52pt 투명 `NSPanel`은 최대 15Hz·1pt 임계값으로 위치를 따라가며 메시지·리액션 클릭과 우클릭을 받는다. `더블 우클릭 후 던지기`가 OFF이면 연결된 활성 방에서 화면에 표시되는 모든 친구마다 Presence 상태와 무관하게 같은 크기의 투명 hotspot을 항상 유지하고, ON이면 내 캐릭터 더블 우클릭 뒤 10초 동안만 만든다. 설정 변경, 오버레이 숨김, 방 전환, Realtime 단절, 현재 사용자 노드 제거 시 10초 상태와 친구 hotspot을 즉시 재구성한다. 전역 마우스 hook·전역 좌표 수집·클릭 재주입은 사용하지 않는다.

설정 토글 제목은 `더블 우클릭 후 던지기`, 설명은 `끄면 친구 캐릭터를 바로 클릭할 수 있고, 켜면 내 캐릭터를 더블 우클릭한 뒤 10초 동안만 클릭할 수 있습니다.`다. 기본값은 OFF다. 따라서 OFF에서는 친구 캐릭터 52×52 영역이 뒤 앱 클릭을 가로채지만 월드의 나머지 투명 영역은 계속 클릭을 통과시킨다.
...
- 탐색 푸터에는 `설정`과 `정보`만 순서대로 배치하고 연결 상태 메뉴나 카드를 표시하지 않는다. 연결 상태는 `정보` 페이지 첫 항목에서 초록 점과 `연결됨`, 단절 상태는 빨간 점과 `연결 안 됨`으로 표시하며 단절 시 같은 카드의 `지금 다시 연결` 버튼으로 수동 복구를 시작한다. 최초 연결과 정상 연결 뒤의 재단절 모두 Windows 시스템 알림을 15초 유예하며 그 안에 복구되면 예약을 취소한다. 15초가 지나도 연결되지 않으면 장애당 한 번 알리고, 짧은 연결 반복에 따른 재알림은 15분 동안 억제한다.
...
- Windows `내 프로필`은 캐릭터 아래에 말풍선·투척물 선택 영역을 항상 표시한다. 각 영역은 `기본 말풍선` 또는 `캐릭터 기본 투척물`을 먼저 보여주고 snapshot의 활성 entitlement가 있는 상품만 추가한다. 캐릭터·말풍선·투척물 선택은 각각 별도 서버 action으로 즉시 적용하고 서버 성공 응답만 반영하며 진행 중인 같은 종류의 중복 요청을 막는다. 닉네임은 정규화한 현재 값과 draft가 다를 때만 `닉네임 변경하기` 버튼을 입력란 가까이에 표시하고 유효한 2~8자 값만 명시적으로 저장한다.
- Windows `꾸미기·상점`은 `캐릭터 / 말풍선 / 투척물` 종류별로 캐릭터 7종·말풍선 3종·투척물 14종을 표시한다. 처음에는 캐릭터와 기본순을 선택하고, 기본순·가격 낮은순·가격 높은순 정렬과 활성 entitlement 기준 `보유 중 숨기기` 필터를 제공한다. 조건에 맞는 상품이 없으면 종류나 보유 필터를 바꾸도록 빈 상태를 안내한다. 모든 카드는 선택 가능한 상세 미리보기 대상으로 유지하고 Release는 `구매 준비 중` 안내만 표시하며 구매·결제·소유권 변경 action을 만들지 않는다. 명시적으로 opt-in한 개발 빌드의 기존 staging 캐릭터 결제 범위는 별도 계약을 유지한다.
- Windows 상세 미리보기는 실제 `UpdateLayeredWindow`용 asset cache와 30 FPS 합성 규칙을 재사용한 두 캐릭터 무대다. 캐릭터 상품은 걷기와 친구 클릭 시 시그니처 투척을, 말풍선 상품은 타이핑 점과 두 본문 순환을, 투척물 상품은 0.35초 뒤 시작하는 좌우 교대 투척을 보여준다. 기본 간격은 1초이며 비행과 충돌이 더 길면 완료까지 기다린다. 미니 대포는 전경 emitter·심지탄·몸통 폭발을 사용한다. 상세를 닫으면 timer·frame cache 참조를 해제하고 모션 감소 설정에서는 정적 첫 장면만 표시한다.
- Windows 후속 상품 구현은 공용 `assets/v1/commerce-catalog.json`의 24개 상품을 앱에 묶어 사용해요. 수달·돼지·나무와 눈송이·야구공·왁구공·두쫀쿠, 애장품 7종을 같은 렌더러와 프로필 장착 경로에 추가해요. 애장품 entitlement는 캐릭터 소유권과 독립적이며, 캐릭터 구매만으로 애장품을 보유한다고 판정하지 않아요. 미장착 기본값은 모든 캐릭터의 패치 말랑공이에요. 서버의 활성 entitlement가 회수되면 기존 snapshot 처리로 기본값에 복귀해요.
- 캐릭터 상세는 캐릭터 카드와 관련 애장품 카드를 나란히 보여줘요. 각 카드의 가격·보유 상태를 따로 표시하고 애장품에는 별도 판매 안내를 붙여요. 미리보기의 소리 버튼은 해당 상세에만 적용하며 전역 소리 OFF를 해제하지 않아요. 나무 미리보기의 단일 우클릭은 상세를 닫을 때 사라지는 산책 정지·재개 상태예요. 실제 오버레이의 내 나무는 단일 우클릭으로 `TreeMovementPaused`를 기기에 저장하고, 방과 앱을 다시 열어도 유지해요. OS의 더블클릭 판정 시간 안에 두 번째 우클릭이 오면 단일 동작을 취소하고 던지기를 활성화해요.
- 두쫀쿠의 충돌 프레임 시간은 0.08·0.10·0.28·0.12초로 총 0.58초예요. 다른 투척물의 충돌은 총 0.24초를 유지해요. 반복 미리보기는 비행과 충돌이 끝날 시간을 보장하므로 두쫀쿠의 마지막 프레임을 다음 투척이 자르지 않아요. 상품 소개와 새 조작 안내는 기존 7개 언어로 제공해요.
- Windows 그룹 설정의 펼친 각 카드에는 `그룹 나가기`를 이름 변경·삭제와 같은 하단 작업 영역에 표시하고, 하단 작업 영역 뒤에 다음 그룹과 구분되는 세로 여백을 둔다. 나가기·이름 변경·삭제·추방은 다른 그룹 mutation 중 함께 비활성화한다.
...
- 월드는 WinUI XAML 창에 투명 표현을 위임하지 않고 `WS_POPUP` 기반 전용 Win32 HWND가 소유한다. 무료 5종과 유료 7종은 24px 원본에서 정수 nearest-neighbor로 만든 premultiplied BGRA frame을 같은 `UpdateLayeredWindow` 렌더러로 표시하며 tick마다 bitmap이나 surface를 새로 할당하지 않는다.
...
- 내 캐릭터 상호작용 52×52 hotspot은 별도 HWND가 소유하고 최대 15Hz·1 DIP 임계값으로 위치를 갱신한다. `더블 우클릭 후 던지기`가 OFF이면 화면에 표시되는 친구별 hotspot HWND를 Presence 상태와 무관하게 계속 유지하고, ON이면 내 캐릭터 더블 우클릭 뒤 10초 동안만 만든다. Windows 셸 표면 위에서는 캐릭터 클릭·우클릭을 받지 않는다. 전역 마우스 hook·전역 좌표 수집은 금지한다.
- composer는 제목 표시줄 없는 400×56 DIP 별도 WinUI 창이며, 내 캐릭터 클릭·트레이 `메시지 작성`만 이 창을 활성화한다. 기본 presenter를 교체하거나 제목 표시줄 설정을 변경하지 않고 Win32 비클라이언트 영역 계산으로 전체 창을 입력 영역으로 사용한다. 입력란 로드 시 포커스를 재요청하고 ×·Esc 닫기는 창을 숨겨 draft를 보존한다. 실제 창 종료 시 프레임 콜백을 해제하며, 반복 열기·닫기·재생성에서 크기·포커스·입력 보존을 검증한다.
- 트레이 메뉴는 맨 위에 굵은 기본 항목 `열기`를 두고 오버레이, 메시지 작성, 활성 그룹, 조용히 모드, 최근 기록, 상점, 그룹 설정, 로그인 실행, 업데이트 확인, 설정, `종료`를 제공한다. 트레이 아이콘 왼쪽 클릭과 연결 실패 알림 클릭도 기본 창을 연다. 트레이의 업데이트 확인은 설정 창을 열거나 활성화하지 않고 백그라운드에서 실행하며, 완료되면 현재 언어의 Windows 시스템 알림으로 최신 버전, 사용 가능한 버전 또는 확인 실패 결과를 표시한다. 사용 가능한 버전 알림에는 버전 번호를 포함한다. 익명 세션을 사용하는 Windows판에는 로그아웃 메뉴를 두지 않는다.
- 앱은 Pages의 Windows 전용 manifest를 시작 시 한 번 확인하고 트레이·정보 페이지에서 수동 확인도 제공한다. 새 버전이 있으면 Windows 시스템 알림을 보내며 정보 페이지의 업데이트 카드는 현재 버전, 마지막 성공 확인 시각, `업데이트 확인` 버튼과 `릴리스 정보` 링크를 표시한다. 링크는 현재 확인된 새 버전 또는 설치된 버전의 `https://github.com/sidey-app/SIDEY/releases/tag/windows-v<version>`만 기본 브라우저로 연다. 설치된 버전보다 새로운 고정 GitHub Release Setup EXE URL과 SHA-256이 모두 유효할 때만 사용자 승인을 받아 내려받고 hash 검증 뒤 설치기를 실행하며 무인 자동 설치는 하지 않는다.
- Windows 정보 페이지는 서버 연결 상태, 언어, 테마, 업데이트와 피드백, 진단 데이터 내보내기, 정책 순으로 표시한다. 연결 상태는 `서버와 연결됨` 또는 `서버와 연결 안 됨`으로 표시하고 별도 동기화 설명은 두지 않으며 재시도 버튼은 `재연결`로 표시한다. 테마 드롭다운은 220 DIP 폭이고 `Windows 색 설정`은 `ms-settings:colors`를 연다. 저장된 테마는 기본 창·메시지 입력 창·최근 기록 창에 함께 적용한다. 메시지 입력 창은 400×56 전체 루트를 테마 표면 한 장으로 채우고 별도 안쪽 배경이나 바깥 여백을 두지 않는다. 밝게 모드는 fallback·tint가 `#F9F9F9`인 밝은 WinUI 인앱 아크릴을 사용하고 어둡게 모드는 기존 어두운 인앱 아크릴을 사용한다. 고대비에서는 아크릴 대신 `SystemColorWindowColor`를 따르는 단색 브러시를 사용하며, 입력 컨트롤의 기본·hover·focus 배경은 투명하게 유지한다. 저장된 밝게·어둡게 테마는 트레이 우클릭 메뉴에도 각각 강제 밝은·어두운 Windows 메뉴 모드로 적용하며 Windows 기본값은 시스템 앱 테마를 따른다. 피드백은 `https://github.com/sidey-app/SIDEY/issues/new/choose`, 정책은 각각 `https://sidey-app.github.io/SIDEY/terms/`, `/privacy/`, `/refund/`를 기본 브라우저로 연다. 진단 내보내기는 `%LOCALAPPDATA%\SIDEY\Logs` 최상위의 `SIDEY.*.log`만 ZIP으로 압축해 바탕화면의 충돌하지 않는 이름에 저장하고 설정·자격 증명·메시지 내용이나 그 밖의 파일을 추가하지 않는다. 설정·정보를 포함한 공통 카드 표면은 WinUI 테마의 카드 배경·테두리, 8 DIP 모서리와 20 DIP 안쪽 여백을 사용한다. 각 페이지의 스크롤바는 제한 폭 콘텐츠 안이 아니라 탐색 본문 오른쪽 끝에 표시하고, 카드 콘텐츠만 820 DIP 최대 폭과 32×24 DIP 여백으로 가운데 배치한다. 그룹 페이지는 스크롤 뷰포트 전체 폭을 먼저 확정한 뒤 같은 제한 폭을 가운데 배치해 방 카드 내용의 필요 폭 때문에 다른 페이지보다 안쪽으로 밀리지 않는다. 그룹 목록의 각 방은 헤더와 구분된 본문을 가진 독립 확장 카드로 표시하며 펼치기·접기에는 높이·불투명도·세로 이동 애니메이션을 적용하고 Windows가 애니메이션을 끈 경우 즉시 전환한다. 접힌 카드에는 아래쪽 꺾쇠, 펼친 카드에는 위쪽 꺾쇠를 사용한다. 꺾쇠는 별도 버튼 표면을 두지 않고 독립 동작 버튼을 제외한 헤더 전체가 확장 영역이며 그룹명·초대 코드·상태·동작이 놓인 헤더의 20 DIP 바깥 여백까지 같은 hover 표면으로 채운다. 초대 코드 복사 성공 시 해당 카드의 버튼에 `복사됨`을 3초 동안 표시한다. 상점 상품은 균일 그리드 셀의 기존 카드 크기를 유지하는 단일 미리보기 버튼이어서 hover와 클릭 범위가 카드 전체와 일치한다. 기본 창에서 페이지를 다시 방문하면 대상 스크롤 위치를 최상단으로 초기화한다.
- Windows 설정의 기본 캐릭터 5종과 기본 말풍선·투척물은 로컬 카탈로그와 저장된 무료 캐릭터 선택을 사용해 서버 연결 전에도 표시한다. 저장된 유료 캐릭터는 권한 확인 전에는 선택 표시를 하지 않는다. 첫 서버 스냅샷과 소유권 상태가 확정될 때까지 추가 유료 선택지 자리와 그룹·상점 결과처럼 네트워크 결과에 의존하는 부분만 스켈레톤을 표시한다. 스켈레톤은 테마 리소스 색을 사용해 밝기를 반복하고 실제 원격 콘텐츠는 짧은 진입 전환으로 나타나며, Windows 애니메이션 설정이 꺼져 있으면 반복과 전환을 생략한다. 페이지 제목·일반 설정·상점 종류 탭과 필터 조작은 기다리지 않고 즉시 표시한다. 그룹 꺾쇠와 상점 필터 꺾쇠는 펼침·접힘에 맞춰 180도 회전한다. 상점 필터 영역은 높이·불투명도·세로 이동으로 열고 닫으며, 필터 버튼은 닫혔을 때 텍스트·아이콘만 표시하고 열렸을 때만 중립 기본 배경과 테두리를 사용한다. 검색 아래의 중립 버튼 `모두 다시 설정`은 현재 상품 종류를 유지한 채 검색어를 빈 값, 정렬을 기본순, 보유 상품 숨기기를 끔으로 되돌린다. 상점 미리보기는 앱 안의 모달 표면으로 표시하며 바깥 음영 영역·닫기 버튼·Escape로 닫으면 실행 중인 미리보기 작업을 정리한다.
- 앱 실행 한 번의 startup과 running 진단은 `%LOCALAPPDATA%\SIDEY\Logs\SIDEY.<underscore-version>.<yyyyMMdd>.<HHmmss>.log` 한 파일에 UTC(+00:00)로 기록한다. 앱·build·Windows·runtime·process architecture와 시작·정상 종료·다음 실행에서 판정한 비정상 종료, 설정·인증·서버·Realtime 구독, 연결·재연결 횟수/대기·WebSocket 종료 코드·HTTP 상태·timeout/DNS/TLS·마지막 정상 수신 경과, 메시지 변경·RLS 재조회·활성 그룹 판정·ledger·말풍선·오버레이 전달/표시, 익명 모니터 번호·DPI·Z-order, 1분 단위 메모리·frame 시간·queue·handle/GDI·UI 응답 지연, 예외 유형·HRESULT·stack·동일 오류 반복 횟수를 기록한다. source stack의 로컬 경로는 제거하고 token·메시지 본문·닉네임·email·사용자/방/메시지 UUID 원문·평문 초대 코드·입력 키·마우스 좌표·화면 내용·활성 앱 목록·파일 내용·Credential Manager 데이터·컴퓨터/사용자 이름은 기록하지 않는다. 파일은 4MB에서 추가 기록을 중단하고 30일·100개·총 32MB 상한으로 정리한다. Windows Realtime watchdog은 5초마다 확인하며 마지막 수신 후 15초 이상 무응답이면 연결을 재생성한다. WinUI 창 생성 전 실패하면 네이티브 오류창으로 로그 경로를 알리고, 트레이 아이콘 초기화만 실패한 경우 설정창은 계속 유지하며 창을 닫으면 앱을 종료한다.
...
- 서버 프로필이 확정한 장착 말풍선·투척물과 종류별 장착 요청 상태
...
`OverlayWindowGroup`은 composer 표시 상태, 내 캐릭터 단일·더블·우클릭 패널, `더블 우클릭 후 던지기` 설정에 따른 10초 타이머와 최대 11개 친구 hotspot 생명주기를 소유한다. `AppCoordinator`는 `character_pulse` 송수신과 캐릭터별 1초 쿨타임, throw 대상 검증·0.5초 쿨타임·이벤트 생성·로컬 재생·서버 호출을 소유한다. `AppModel`에는 영구 `requiresRightClickToThrow`만 저장하고 화면 좌표, 활성화 타이머나 애니메이션 frame을 저장하지 않는다.
...
- `Sidey.Overlay`는 전용 Win32 message-loop thread와 30FPS 고정 step을 소유하고 불변 `WorldSnapshot`을 UUID diff로 반영한다. 위치·속도·animation frame은 앱 세션 상태에 저장하지 않는다. 무료 5종과 유료 7종 기본 시트, 승인된 throw/hit·물체 PNG를 BGRA frame으로 시작할 때만 변환·캐시하고 종료 시 모두 해제하며 tick마다 bitmap·surface·projectile buffer를 새로 할당하지 않는다. Debug 검증 모드는 같은 렌더러의 캐릭터 목록만 햄스터로 제한한다. 유료 source character 이벤트는 해당 캐릭터의 action과 서버가 보낸 장착 투척물을 표시한다. 미장착 투척물은 캐릭터와 무관하게 패치 말랑공으로, 알 수 없는 캐릭터 ID는 햄스터로 fallback한다.
...
공개 웹사이트의 `/store/`와 `/store/characters/`는 같은 캐릭터 상품 화면을 제공하고, `/store/throwables/`와 `/store/bubbles/`를 포함한 세 종류를 콘텐츠 폭에 꽉 차는 `캐릭터·투척물·말풍선` 탭으로 전환한다. 기본 캐릭터 5종, 기본 `patch_soft_ball`과 기본 흰색 말풍선은 각 탭에서 추가 상품보다 먼저 보여 주고 가격 자리에 `0원`이 아닌 `기본 제공`을 표시한다. 상품은 3열 카드로 나누어 표시하고 각 카드는 픽셀 이미지를 여유 있는 안쪽 공간에 배치하고 로딩 중 스켈레톤을 보여준다. 닫힌 카드에서는 상품 애니메이션을 재생하지 않고 사용자가 선택한 모달 안에서만 캐릭터의 2~5번 걷기 프레임을 0.16초 간격으로 재생한다. 앱 manifest에서 진행 방향 mirror가 선언된 캐릭터만 좌우 벽에서 진행 방향으로 반전하고 나머지 정면형 캐릭터는 뒤집지 않으며, 표시 속도는 앱의 22pt/s를 미리보기 배율에 맞춘다. 투척 미리보기는 던지는 캐릭터의 0~3번 action, 회전하는 투척물의 이차 포물선, 맞는 캐릭터의 4~7번 hit와 투척물 8~11번 impact를 함께 재생한다. 미니 대포의 닫힌 카드는 대포알 첫 프레임만 보여 주고 모달에서는 발사 캐릭터·정식 emitter·대포알·피격 캐릭터를 모두 합성한다. 말풍선은 앱 manifest의 배경·테두리·글자색과 픽셀형 꼬리를 따르되 캐릭터 애니메이션 없이 장식과 이미지에 포함되지 않은 HTML 글자만 보여 준다. 미리보기 모달은 열고 닫을 때 220ms 이하의 opacity·scale 전환을 사용한다. 웹에서는 직접 구매 action을 제공하지 않고 구매와 장착이 macOS SIDEY의 꾸미기·상점에서 이루어진다고 안내한다. 상점은 별도 구매 안내 섹션, 결제·개인정보 설명 카드와 판매자 정보를 반복하지 않고 랜딩과 같은 공통 푸터에서 정책 링크를 제공한다. 한국어 제목과 본문에는 적절한 폭과 `word-break: keep-all`, `text-wrap`을 적용해 한 글자만 다음 줄에 남는 줄바꿈을 막고, 상점 타이포그래피는 홈 랜딩보다 작은 페이지 전용 크기를 사용한다. 상품 소개에는 production·staging·Sidey-dev·테스트 채널·출시 준비 상태 같은 내부 운영 정보를 노출하지 않는다. 새로운 기능 화면도 같은 전체 폭 탭을 사용하며 `/whats-new/`와 `/whats-new/macos/`는 macOS, `/whats-new/windows/`는 Windows 정식 GitHub Release를 빌드 시 GitHub API에서 최대 100건까지 읽는다. 카드에는 GitHub Release 자체 제목과 별도 최신 버전 hero 문구를 반복하지 않고 버전·본문·공개일·원문 링크를 표시하며, 최신 하나만 펼치고 지난 버전은 접은 상태로 세 건씩 무한 스크롤한다. 영어 경로에는 `/en/` 접두사를 붙인다. 모든 일반 공개 페이지의 푸터는 랜딩 푸터를 재사용하며 현재 메뉴 active 상태와 상점·GitHub 링크를 표시하지 않는다. 이용약관과 개인정보 처리방침의 시행일은 상단 보조 문구가 아니라 본문 조항으로 둔다. 두 화면 모두 고정 상단 바 아래의 소개 영역 높이와 제목 크기를 필요한 만큼만 사용해 첫 화면에서 탭과 첫 콘텐츠 일부를 함께 보여 준다. Astro가 생성한 `/checkout/`과 `/checkout-result/`은 상품 ID별 이름·가격·이미지를 사용하고 공개 구매 링크로 노출하지 않으며 staging/dev 주문 token으로만 접근한다. 결제 카드 정보는 SIDEY가 수집하지 않고 PortOne을 통해 열린 실제 PG 결제창이 처리한다.
...
- Realtime: current epoch topic 접근, client DB형 Broadcast 거부, 다른 사용자·방 transient event 거부, bounded stream overflow 재동기화, macOS·Windows OS 경로 단절·복귀 전환 coalescing과 350ms 복구 debounce, Windows 5초 heartbeat·15초 수신 무응답 판정, DB·ephemeral 채널 쌍 판정, 토큰 교체 뒤 `system` 오류·`phx_error`·`phx_close` 채널 종료의 즉시 연결 해제와 전체 재구독, 유효한 기존 토큰이 있는 동안 일시적인 세션 갱신 실패 뒤 동일 소켓 heartbeat 유지·재시도, 만료 토큰과 명확한 인증 실패의 연결 해제·중단, stale generation callback 거부, 전체 topology backoff와 snapshot reconciliation, 전체 `presence_state`에서 누락된 기존 사용자의 오프라인 보정, Presence publication 최대 동시 실행 수 1, 활성 방 transport 복구 뒤 다른 방·후속 보정 중 자기 캐릭터의 회색 상태와 던지기 비활성 해제를 검증한다. `character_throw`는 미인증·누락 UUID·stale epoch·송신자/대상 비멤버·자기 자신 대상·10초 20회 초과를 거부하고 payload에 좌표나 클라이언트 지정 source character가 없는지 확인한다. 실서버 2클라이언트에서는 프로필을 즉시·10초·30초 간격으로 연속 변경해 transport 단절 이벤트가 생기지 않는지 확인하고, 별도로 네트워크 unavailable→available 전환·강제 단절·추방 뒤 자동 재구독·메시지·Presence·`character_pulse`·`character_throw` 격리를 확인한다.
...
- 창: 월드 항상 위·나머지 영역 클릭 통과, 내 캐릭터 52×52 hotspot, 기본 OFF에서 화면에 표시되는 친구별 Presence 상태 무관 52×52 상시 hotspot, ON에서 더블 우클릭 전 통과·더블 우클릭 뒤 10초 활성화·재더블 우클릭 갱신·만료, 설정 전환·숨김·방 전환·단절 시 즉시 재구성, composer의 선택 모니터 상단 중앙·노치 아래 10pt 배치와 멀티 데스크탑 현재 Space 이동, 왼쪽 `×`·Esc·외부 클릭 닫기, 단일·더블클릭 회귀와 throw 0.5초 쿨타임, composer 초기 숨김·열기·마지막 전송 뒤 5초 자동 닫힘·타이머 갱신·실패 복구, 기록 일반 창
...
1. 무료 5종과 원격 유료 7종, 말풍선 3종과 투척물 14종을 하나의 `UpdateLayeredWindow` 렌더링 경로로 제공하고 asset·frame·발 기준선·방향·fallback 계약, 별빛 우파루파의 catalog 기반 sparkle 효과를 자동 검증한다. 선택 목록은 무료 5종·활성 entitlement 캐릭터와 기본값·보유 말풍선·보유 투척물을 보여준다. 성공한 snapshot에서 장착 상품 entitlement가 회수되면 캐릭터는 햄스터, 말풍선·투척물은 종류별 기본값으로 fallback해야 한다.
2. 상점의 캐릭터 7종·말풍선 3종·투척물 14종 카드는 모두 상세 무대를 열고 실제 걷기·타이핑·본문·시그니처 투척·일반 투척·대포 전용 연출을 재생한다. Release는 구매 action을 만들지 않으며 상세를 반복해서 열고 닫은 뒤 timer·surface·frame cache 참조가 남지 않아야 한다.
...
투척물 미리보기 15종은 앱과 동일한 충돌음을 제공한다. 소리 토글은 기본 꺼짐이고, 켜면 즉시 한 번 재생한 뒤 애니메이션 충돌 시점에 재생한다. 모션 감소 설정에서는 토글을 켤 때만 재생하며, 소리를 끄거나 상세를 닫거나 다른 상품을 열거나 페이지가 숨겨지면 기존 재생을 중지한다. 버튼은 SVG 스피커·음소거 아이콘과 켜짐 색을 사용한다.

상점·checkout·결과 화면은 `commerce-catalog.json`에서 생성한 동일한 24종 정의를 사용한다. checkout 표시는 서버가 확인한 주문명·가격을 유지하며 이미지 경로는 배포 base를 보존한다. 기본 제공 7종은 유료 상품 차집합이 아닌 명시 목록이고, 영어·일본어 유료 상품 번역은 한 파일에서 관리하며 누락 시 빌드에 실패한다. 서버의 판매 잠금·가격 재검증·권리 지급 조건은 유지한다. 상품 생성·검사 절차는 `assets/README.md`를 따른다.
...
- FAQ와 새로운 기능의 `details` 본문은 모션 감소 설정이 아닐 때 280ms 높이·opacity 전환으로 위에서 아래로 펼쳐지고 같은 방식으로 접힌다. 새로운 기능의 최신 항목은 처음부터 펼친 상태를 유지한다.
- 모바일 랜딩은 닫힌 상단 바와 히어로 제목이 붙어 보이지 않도록 제목 위에 여유를 둔다. 모바일 새로운 기능 화면은 소개·탭과 첫 변경 카드가 한 흐름으로 보이도록 두 영역 사이의 상하 여백을 줄인다. FAQ 목록은 첫 질문 위와 마지막 질문 아래의 외곽 구분선을 생략한다.
- 상점의 캐릭터 모달은 실제 오버레이와 같이 walk 2~5번 프레임을 각각 0.16초 동안 보여 주며, 표시 배율을 반영한 22pt/s 속도로 좌우 벽 사이를 왕복한다. manifest에서 진행 방향 mirror가 선언된 캐릭터만 방향에 맞춰 반전하고 닫힌 상품 카드는 정지 상태로 둔다.
- 상점 말풍선은 배포 asset의 128×48 preview가 사용하는 계단형 모서리·픽셀 외곽선·꼬리와 테마 색을 CSS로 재구성한다. 장식 PNG와 이미지에 포함되지 않은 HTML 글자, 정지한 햄스터를 카드와 모달에 합성한다. 말풍선 너비는 표시 문구 길이에 맞춰 늘리고, 언어에 따라 폭이 달라져도 꼬리는 중앙에 둔다.
- 상점은 한국어 `/ko/store/`, 영어 `/en/store/`, 일본어 `/ja/store/` 및 각 언어의 카테고리 경로를 같은 구조로 제공하고, 상품명·설명·상태 문구는 각 언어권 사용자에게 자연스러운 별도 문구로 관리한다.
- 공통 상단 바의 기능·다운로드·FAQ는 언어별 홈 절대 해시 URL을 사용한다. 700px 이하에서는 햄버거 버튼만 보이고, 버튼을 누르면 상단 바와 같은 블러 표면의 세로 메뉴가 펼쳐지며 메뉴가 열린 동안 배경 문서 스크롤을 잠근다.
- 모바일 메뉴는 브랜드와 햄버거가 있는 첫 행을 같은 위치에 고정한 채 표면과 항목을 위에서 아래로 펼치고 접는다. 랜딩의 운영체제 다운로드 선택 메뉴도 같은 방향의 opacity·transform 전환을 사용한다.
- 시스템 다크 모드에서는 랜딩 모니터 안의 브라우저 화면도 어두운 도구 막대·본문·자리표시선으로 전환한다. 푸터 정책 링크의 밑줄은 hover와 키보드 focus 때만 왼쪽에서 오른쪽으로 그려진다.
- 공개 `/ko/terms/`와 `/ko/privacy/`는 각각 `website/src/pages/ko/terms.md`, `privacy.md` 원문을 공통 Astro Markdown 정책 layout으로 렌더링한다. 이용약관은 서비스 조건과 소프트웨어 설치·실행 사용권을 한 문서로 제공하고 별도 라이선스 페이지나 운영자 문의 조항을 두지 않으며 Windows 설치기도 같은 Markdown 원문을 사용한다. 메시지 길이·보관 기간·그룹 인원·지원 환경·로그인 및 결제 사업자처럼 변경 가능한 제품·운영 사양은 이용약관에 고정하지 않고 해당 앱·웹 안내, 개인정보 처리방침 또는 환불 정책에서 관리한다. 개인정보 처리방침은 실제 처리 목적·항목, 수집 경로, 3일 메시지 보관과 법정 거래 기록, 제3자 제공과 국외 처리, 파기, 쿠키, 이용자 권리와 보호 조치를 표가 아닌 목록 중심으로 구분한다. 두 문서의 시행일은 2026년 9월 4일로 유지한다.
- 이용약관·개인정보 처리방침·청약철회 및 환불 정책은 마지막 조항 아래에 별도 구분선을 두지 않는다. 개인정보 보호책임자 항목에는 책임자 이름과 이메일만 표시한다.
