...
출시 전 검토 자료: PR #27의 시바견·오리·똥·떡볶이·쿼카 5종은 [character-five 검토 패키지](reviews/character-five/README.md)에서 원본과 후보를 구분해 검토한다. 채택 대상 선별·서면 계약 완료·실루엣 보강 방향과 시바 테니스공·오리 물방울·똥 휴지 뭉치·떡볶이 어묵꼬치 기획이 확정됐다. 실제 24×24 기본 자세는 승인 대기이며 쿼카는 잎사귀·풀 뭉치를 비교한 뒤 선택한다. 전체 동작·물건 그림·충돌음은 단계별 승인 대상이다. 전 단계 승인 후에도 이번 결과물은 출시 대기 자료이며 현재 지원 캐릭터·상품 수에 포함하지 않는다. 각 캐릭터의 기본 10·동작 8프레임과 선택 물건별 회전 8·충돌 4프레임, 물건별 단일 충돌음을 준비하고 승인 해시를 보존한다. 이후 macOS 업데이트 지시 전에는 활성 manifest·앱·상점·서버·플랫폼 배포 사본에 연결하지 않는다.
